...
export function createPlanner(config, store) {
  ...
  function synchronizeTaskGroups(plan) {
    if (!plan || !Array.isArray(plan.ieltsTasks)) {
      return plan;
    }

    const plannedIeltsTasks = plan.ieltsTasks.filter(
      (task) => task.source === "planned" || task.source === "core"
    );
    const carryOverTasks = plan.ieltsTasks.filter((task) => task.source === "carry-over");
    const extraPracticeTasks = plan.ieltsTasks.filter((task) => task.source === "adaptive");
    ...
    return {
      ...
      carryOverTasks,
      extraPracticeTasks,
      progress: summarizeProgress(plan.date, plan.ieltsTasks, plan.phase)
    };
  }
  ...
  function ensureDay(dateString, options = {}) {
    ...
    if (existing && !options.forceRefresh) {
      ...
      const normalizedPlan =
        existing.planVersion === PLAN_SCHEMA_VERSION
          ? synchronized
          : { ...synchronized, planVersion: PLAN_SCHEMA_VERSION };
      if (JSON.stringify(normalizedPlan) !== JSON.stringify(existing)) {
        store.savePlan(dateString, normalizedPlan);
      }
