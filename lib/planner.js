...
export function createPlanner(config, store) {
  ...
  function getLawSlot(dateString) {
    const activeStage =
      ...
      null;
    const objectiveExamDate = "2026-09-12";
  ...
  function buildIeltsTasks(dateString, phase) {
    const baseTasks = (weeklyStructure[String(getDayIndex(dateString))] ?? []).map((task) => ({
      ...task
    }));

    const intensity = normalizeIntensity(phase.intensity);
    let taskSet = baseTasks;

    if (intensity === "light") {
      taskSet = buildLightTasks(baseTasks);
    } else if (intensity === "heavy") {
      taskSet = buildHeavyTasks(baseTasks);
    }

    const tasks = taskSet.map((task, index) => attachTaskMetadata(task, index, "core", intensity ?? "normal"));

    const busyProfile = getBusyProfile(dateString);
    if (busyProfile?.ieltsModifier === "light" && intensity !== "heavy" && tasks.length > 1) {
