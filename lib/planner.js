...
export function createPlanner(config, store) {
  ...
  function attachPlannedTaskMetadata(task, dateString, index, source, intensity) {
    ...
    return {
      ...
      carryKey: `ielts:${dateString}:${slugify(withMetadata.title)}`
    };
  }
  ...
  function createLawTasksForDate(dateString, lawSubjects, source = "planned", carriedFrom = null) {
    ...
    return (lawSubjects ?? []).map((subject, index) => {
      ...
      return {
        ...
        originalDate,
        carryKey: `law:${originalDate}:${slugify(subject)}`
      };
    });
  }
  ...
  function normalizeHistoricalLawTasks(planDate, plan) {
    if (Array.isArray(plan?.lawTasks) && plan.lawTasks.length) {
      return plan.lawTasks.map((task, index) => ({
        ...
        carryKey:
          task.carryKey ??
          `law:${task.originalDate ?? task.carriedFrom ?? planDate}:${slugify(task.title ?? task.subject ?? `law-${index}`)}`
      }));
    }
    ...
    return tasks
      ...
      .map((task, index) => {
        ...
        return {
          ...
          carryKey:
            task.carryKey ??
            `ielts:${originalDate}:${slugify(task.title ?? `ielts-${index}`)}`
        };
      });
  }
  ...
  function buildPlan(dateString) {
    ...
    const carryOverTasks = carryOver.all;
    ...
    const extraPracticeTasks = adaptive.extraTasks
      ...
      domain: "ielts",
      originalDate: dateString,
      carryKey: `adaptive:${dateString}:${slugify(item.task.title)}`,
      phaseNote: "Adaptive add-on"
      }));

    const allIeltsTasks = [...carryOver.ielts, ...plannedIeltsTasks, ...extraPracticeTasks];
    const allLawTasks = [...activeLawCarryOverTasks, ...plannedLawTasks];
    const busyProfile = getBusyProfile(dateString);
    const lawHours = busyProfile?.lawHours ?? appConfig.defaultLawHours;
    const outstandingTasks = [...carryOver.ielts, ...activeLawCarryOverTasks];
    ...
    return {
      ...
      carryOver: carryOverTasks,
      carryOverTasks: [...carryOver.ielts, ...activeLawCarryOverTasks],
      outstandingTasks,
      ieltsCarryOverTasks: carryOver.ielts,
      lawCarryOverTasks: activeLawCarryOverTasks,
      plannedIeltsTasks,
      plannedLawTasks,
      extraPracticeTasks,
  ...
  function ensureDay(dateString, options = {}) {
    ...
    if (existing && !options.forceRefresh) {
      if (existing.planVersion !== PLAN_SCHEMA_VERSION) {
        const rebuiltPlan = buildPlan(dateString);
