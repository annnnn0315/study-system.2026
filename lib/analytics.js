...
function getValidBandValues(entries, keys) {
  const allowed = new Set([5.5, 6.0, 6.5, 7.0, 7.5, 8.0, 8.5, 9.0]);
  const values = [];
...
function buildIeltsProgress(entries, reviews) {
  ...
  const readingAccuracies = entries
    ...
    .filter((value) => !Number.isNaN(value));
  const listeningCount = listeningAccuracies.length;
  const readingCount = readingAccuracies.length;
  const speakingBands = getValidBandValues(entries, ["speakingBand"]);
  const speakingCount = speakingBands.length;

  const averages = {
    listeningAccuracy: averagePrecise(listeningAccuracies),
    readingAccuracy: averagePrecise(readingAccuracies),
    speakingBand: averagePrecise(speakingBands),
  ...
  const listeningBand = listeningCount >= 2 ? mapAccuracyToIeltsBand(averages.listeningAccuracy) : null;
  const readingBand = readingCount >= 2 ? mapAccuracyToIeltsBand(averages.readingAccuracy) : null;
  const speakingBand = speakingCount >= 2 ? Math.round(averages.speakingBand * 10) / 10 : null;
  const writingBand = writing.value;
  ...
  const usableBands = [listeningBand, readingBand, writingBand, speakingBand].filter(
...
function buildIeltsPriority(entries, reviews, overview) {
  ...
  const candidates = [
    ...
    {
      area: "Speaking",
      score: speakingBands.length ? averagePrecise(speakingBands) : -1,
      missing: !speakingBands.length,
      repeated: issueCounts.speaking >= 2,
      reason: !speakingBands.length
        ? "No recent data"
