...
export function renderTodayDashboard({ container, plan, onToggleTask }) {
  ...
  const allLawTasks = Array.isArray(plan.lawTasks) ? plan.lawTasks : [];
  const plannedTasks = allIeltsTasks.filter((task) => task.source === "planned" || task.source === "core");
  const carryOverTasks = allIeltsTasks.filter((task) => task.source === "carry-over" && task.completed === false);
  const extraPracticeTasks = allIeltsTasks.filter((task) => task.source === "adaptive");
  const plannedLawTasks = allLawTasks.filter((task) => task.source === "planned");
  const lawCarryOverTasks = allLawTasks.filter((task) => task.source === "carry-over" && task.completed === false);
  const outstandingTasks = Array.isArray(plan.outstandingTasks)
    ? plan.outstandingTasks
    : [...carryOverTasks, ...lawCarryOverTasks];
  ...
  const renderInteractiveTaskSection = (label, key, tasks, emptyMessage) => {
    if (!tasks.length) {
    ...
    return `
      <div class="task-group">
        <p class="mini-label">${escapeHtml(label)}</p>
        <div class="task-list" data-task-group="${escapeHtml(key)}"></div>
      </div>
    `;
  };

  const renderLawTaskList = (label, tasks, emptyMessage) => {
    if (!tasks.length) {
      return emptyMessage ? `<p class="empty-state">${escapeHtml(emptyMessage)}</p>` : "";
    }
  ...
  const renderMissedSummary = () => {
    const missedIelts = outstandingTasks.filter((task) => task.domain === "ielts");
    const missedLaw = outstandingTasks.filter((task) => task.domain === "law");
    if (!missedIelts.length && !missedLaw.length) {
      return '<p class="empty-state">No missed tasks carried over.</p>';
    }

    return `
      ${renderLawTaskList("IELTS", missedIelts, "")}
      ${renderLawTaskList("Law subject", missedLaw, "")}
    `;
  };
    ...
    <div class="today-layout">
      <div class="today-column">
        ...
        <div class="metric-card">
          <p class="mini-label">Today's Study Plan</p>
          <h3 class="section-title">Carry-over Tasks</h3>
          ${renderInteractiveTaskSection("IELTS", "carry-over-ielts", carryOverTasks, "")}
          ${renderInteractiveTaskSection("Law subject", "carry-over-law", lawCarryOverTasks, "")}
          ${!carryOverTasks.length && !lawCarryOverTasks.length ? '<p class="empty-state">No carry-over tasks for today.</p>' : ""}

          <h3 class="section-title">Today's Scheduled Tasks</h3>
          ${renderInteractiveTaskSection("IELTS", "planned-ielts", plannedTasks, "No planned IELTS tasks for today.")}
          ${renderInteractiveTaskSection("Law subject", "planned-law", plannedLawTasks, "No scheduled law tasks for today.")}

          ${extraPracticeTasks.length ? `<h3 class="section-title">Extra practice</h3>${renderInteractiveTaskSection("IELTS", "extra-practice-ielts", extraPracticeTasks, "")}` : ""}
        </div>
      </div>
  ...
  mountTasks("planned-ielts", plannedTasks);
  mountTasks("planned-law", plannedLawTasks);
  mountTasks("carry-over-ielts", carryOverTasks);
  mountTasks("carry-over-law", lawCarryOverTasks);
  mountTasks("extra-practice-ielts", extraPracticeTasks);
}
...
export function renderTrackingForm({ container, tracking, lastUpdatedAt, saveFeedback, onSubmit }) {
  container.innerHTML = `
    <div class="form-grid">
      ${createInputGroup({
