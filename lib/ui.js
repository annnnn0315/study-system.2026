...
export function renderTodayDashboard({ container, plan, onToggleTask }) {
  ...
  const plannedTasks = plan.plannedIeltsTasks ?? plan.ieltsTasks.filter((task) => task.source === "planned" || task.source === "core");
  const carryOverTasks = plan.carryOverTasks ?? plan.carryOver ?? [];
  const extraPracticeTasks = plan.extraPracticeTasks ?? plan.ieltsTasks.filter((task) => task.source === "adaptive");
